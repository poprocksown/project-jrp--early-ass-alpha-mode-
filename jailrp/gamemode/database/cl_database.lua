...
function SKINS:DrawFrame(w, h)
	...
	draw.RoundedBoxEx( rounded, 0, topHeight, w-4, h-topHeight-2, SKINS.COLORS.goodBlack, false, false, true, true)

	local QuadTable = {}
	QuadTable.texture = surface.GetTextureOD("gui/gradient")
...
function inventoryMenu()
	...
	local items = vgui.Create("DPannelList", ps)
	...
	local function ItemButtons()
		for k,v in pairs(inventory) do
			...
			if i then
				...
				--add use fuction here
				f:Close()
			end)

				buttons["drop"] = (function()
					--add drop function here
					f:Close()
			end)

			local b = inventoryItemButton( k, i.name .. "(" .. v.amount .. ")", i.desc, i.model, items, i.buttonDist, buttons)
			items:AddItem(b)
		end
	end
